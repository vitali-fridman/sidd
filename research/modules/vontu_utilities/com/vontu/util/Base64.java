...
public class Base64
{
    ...
    public static byte[] decode(final byte[] source, final int off, final int len) throws IllegalArgumentException {
        ...
        for (i = off; i < off + len; ++i) {
            ...
            if (sbiDecode < -5) {
                throw new IllegalArgumentException("Bad Base64 input character at " + i + ": " + source[i] + "(decimal) [" + new String(source, off, len) + "]");
            }
            if (sbiDecode >= -1) {
                b4[b4Posn++] = sbiCrop;
                if (b4Posn > 3) {
    ...
    public static class InputStream extends FilterInputStream
    {
        ...
        public int read() throws IOException {
            if (this.position < 0) {
                if (this.encode) {
                    ...
                    for (j = 0, j = 0; j < 4; ++j) {
                        ...
                        do {
                            b4 = this.in.read();
                        } while (b4 >= 0 && Base64.DECODABET[b4 & 0x7F] <= -5);
                        if (b4 < 0) {
                            break;
                        }
                        b3[j] = (byte)b4;
                    ...
                    if (j == 4) {
                        this.numSigBytes = decode4to3(b3, 0, this.buffer, 0);
                        this.position = 0;
                    }
                    else {
                        if (j == 0) {
            ...
            ++this.lineLength;
            final int b5 = this.buffer[this.position++];

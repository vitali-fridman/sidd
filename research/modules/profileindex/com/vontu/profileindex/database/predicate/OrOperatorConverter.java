...
class OrOperatorConverter implements PredicateConverter
{
    private final MultiColumnPredicateConverter _leafConverter;
    private final OperatorCollectionConverter _operatorConverter;
    
    OrOperatorConverter(final MultiColumnPredicateConverter leafConverter, final OperatorCollectionConverter operatorConverter) {
    ...
    public RowMatcher createMatcher(final PredicateOperator operator) {
        assert operator.operator() == BooleanOperator.OR;
        ...
        return new OrOperator(matchers);
    }
}
